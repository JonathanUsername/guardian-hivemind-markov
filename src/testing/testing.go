...
func Scrape(query string, wg *sync.WaitGroup, c *Chain) {
    ...
    fmt.Println(query)
    url := fmt.Sprintf("http://content.guardianapis.com/search?show-fields=body&page-size=200&api-key=%s&q=%s", getKey(), query) // format without printing
    ...
    arr, err := fromJson(body).Array("response", "results")
    check(err)
    for i :=0; i<len(arr) ;i++ {
        fmt.Println(arr[i])
    }
...
type Results struct {
    AllResults `json:"response.results"`
}
